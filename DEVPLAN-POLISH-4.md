...
- [x] **R4-F2 最近文件 / 最近 App（+ Spotlight 最近区）** [S/M, 价值极高]：全系统无 recents。新 `system/recents.svelte.ts`（`persisted('qz.recents')` 环形封顶 ~20 + pushRecentFile/App），挂 `sys.openApp` 与 Files `open()` 两个 chokepoint；Spotlight 空查询时置顶「最近」区。无头可验。
  - ✅ 实现：新 `system/recents.svelte.ts`（`recents=persisted('qz.recents',{files,apps})` CAP20 + `pushFront` 置顶去重 + pushRecentFile/pushRecentApp/forgetRecent）。**单一 chokepoint**：`sys.openApp` 记最近——文件查看器(textedit/imageviewer/mediaviewer)+`typeof data==='string'`→最近文件；非 hidden 可见 App→最近 App（隐藏宿主 webview/userapp 都不记）。Spotlight 空查询：最近文件解析(getNode,跳过 trash/缺失/root)置顶 5 条 + apps 按最近度 `recents.apps.indexOf` 排序浮前；有查询则 recentFiles=[]、apps 不排序（搜索零回归），列表 slice(0,10)→(0,12) 留头。
