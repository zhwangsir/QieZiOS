...
- [x] **R5-F2 顶栏快捷设置面板（Quick Settings）** [M, 价值高]：顶栏只有 🍆/🌙/🔔/钟。新 `shell/QuickSettings.svelte`（仿 TopBar 托盘下拉）：明暗/auto、声音开关+音量、勿扰、下一张壁纸、主色swatch。承载 F1/F3。
  - ✅ 实现：新 `shell/QuickSettings.svelte`（⚙️ 触发 + 自管 open/el bind + svelte:window 点外部关，镜像通知托盘）。面板：外观(明/暗/自动/定时 4 段→settings.mode)、勿扰(toggle dnd.enabled + 开/关 badge)、声音(toggle soundPrefs.enabled + 启用时显音量 range)、主色(accentPresets 7 色 swatch→settings.accent)、下一张壁纸(nextWallpaper：清 customWallpaper + 循环 wallpaperId)。TopBar 挂 `<QuickSettings/>`（🌙 一键明暗保留）+ **移除独立 🤫**（勿扰移入面板）+ 删 dnd/toggleDnd import。
  - ✅ 浏览器实测：⚙️ 开面板含 5 区、4 mode 钮、7 主色 swatch；主色点选 → --color-qz-accent 实时变；勿扰 badge 关→开→关；声音开 → 音量 slider 现；mode 自动 → settings.mode='auto'；下一张壁纸 → wallpaperId 推进；点外部关闭。0 console error。supervisor 子 Agent PASS（状态接线正确+nextWallpaper 模回绕含 -1 兜底、双 svelte:window 各查自己 el 无冲突、TopBar 移除无残留引用、a11y[swatch aria-label]+ring 有效、shell→system 无环、build 六点），并按其建议删掉死 import resolvedMode。npm check+build 0 错 0 警。
...
> 当前循环：第 5 轮；**C1-C5 + F1（自动明暗）+ F2（QuickSettings）+ F3（勿扰）已完成 ✅**（F1→F3→F2 簇成）。剩 F4/F5/F6/F7/F8（功能）。下一项建议：R5-F4（Files 键盘导航 + F2 改名 + Ctrl+D，日常高频，无头可验）/ R5-F5（壁纸轮播，复用 QuickSettings 下一张）/ R5-F8（窗口置顶）/ R5-F7（更多小组件）/ R5-F6（拖文件进 App）。
