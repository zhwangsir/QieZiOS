...
- [x] **G3.1 环境变量 + 配置**：完善 `env/export/$VAR`；`/etc` 风格配置存储 + shell rc（开机/启动终端时执行）；命令解析的 `$PATH` 雏形。
  ...
  - ✅ 验证：`npm check`+`build` 0 错；supervisor 子 Agent 审查 **PASS**（确认 `.` 别名/`source→run` 无 TDZ、重复开终端不会建 `etc 2`、纯注释 rc 不输出、`which`/`unset`/PATH 边界 OK），并据其建议补了 source 递归守卫。⏳ **待真机验证**（本轮 preview 工具不可用，未做浏览器级 DOM 验证）。
