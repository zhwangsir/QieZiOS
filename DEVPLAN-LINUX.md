...
- [ ] **G2.1 标准流 + 管道**：shell 支持 `\|` 串联、`<` 输入重定向、`2>` 错误重定向；命令模型改成 (stdin)→{stdout,stderr,code}。
- [ ] **G2.2 文本处理 coreutils**：`grep`(含 -i/-n/-r) `find`(按名/递归) `wc` `head` `tail` `sort` `uniq` `cut`，配合管道。

### Phase G3 — 配置与虚拟文件系统
- [ ] **G3.1 环境变量 + 配置**：完善 `env/export/$VAR`；`/etc` 风格配置存储 + shell rc（开机/启动终端时执行）；命令解析的 `$PATH` 雏形。
- [ ] **G3.2 `/proc` `/dev` `/etc` 虚拟挂载**：VFS 路径解析支持只读虚拟节点——`/proc/<pid>` 暴露进程信息为文件、`/dev` 暴露浏览器能力（clipboard/notify 等）、`/etc` 放配置。一个 overlay/mount 机制。

### Phase G4 — 权限与用户
- [ ] **G4.1 权限与所有权模型**：VNode 加 `mode`(rwx) + `owner`；`chmod`/`chown`/`stat` 命令；`ls -l` 显示权限/属主/mtime/大小；操作处 best-effort 校验。
- [ ] **G4.2 用户/账户模型**：当前用户 + 用户表 + `su`/`sudo` 隐喻 + `/etc/passwd` + `whoami`/`id`；与权限联动（默认单用户，可加用户）。
...
3. **监督**：派另一个子 Agent（审查）跑 `npm run check` + `npm run build`，审查 diff 找 bug/回归/架构违背。不过关 → 打回开发 Agent 修（≤3 轮）。
