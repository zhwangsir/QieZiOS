...
- [ ] **H2 别名 + 持久历史**：`alias ll='ls -l'`/`alias`/`unalias`（持久化、启动展开）；命令历史持久化（↑/↓ 跨终端/刷新）。
- [ ] **H3 脚本控制流 + 执行脚本**：`if/then/elif/else/fi`、`for x in … do … done`、`while`、`test`/`[ ]`；`sh <file>` / `./file.sh` 执行脚本文件（带 `#!` 忽略）。
- [ ] **H4 后台作业（job control）**：`cmd &` 后台、`jobs`、`fg`/`bg`、`wait`（在异步 shell 模型里尽力而为）。
- [ ] **H5 从终端定时**：`at <延迟> <命令>` / `crontab`（走 `sys.schedule`，到点经总线跑命令/发通知）。
