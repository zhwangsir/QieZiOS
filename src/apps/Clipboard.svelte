<script lang="ts">
  ...
  function reuse(text: string) {
    ...
    sys.notify('已复制到剪贴板', { level: 'success', timeout: 1500, source: '剪贴板' });
  }
</script>
...
<div class="flex h-full flex-col text-qz-text">
  <div class="flex shrink-0 items-center justify-between border-b border-qz-border px-3 py-2">
    <span class="flex items-center gap-1 text-xs text-qz-muted"><Icon name="📋" size={12} />剪贴板 · {clipboard.items.length}</span>
  ...
  {#if clipboard.items.length === 0}
    <div class="grid flex-1 place-items-center px-6 text-center text-sm text-qz-muted">
      <div>
        <div class="mb-2 flex justify-center"><Icon name="📋" size={36} strokeWidth={1.5} /></div>
