...
| `apps/Studio.svelte` | **开发者 App**（D2）：左 `<CodeMirror>`(语法高亮) + 右 `<Sandbox>` 实时预览 + ▶运行/**💾保存为 App**/重置/SDK 说明。可被「我的 App」以 `data.editAppId` 唤起编辑现有 App |
