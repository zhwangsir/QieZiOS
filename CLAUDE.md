...
| `lib/shell.ts` | **Shell 引擎**(对标 Linux · 终端用)：分词(尊重引号) + `$VAR` 替换 + **管道 `\|` + 重定向 `< > >> 2>`**(命令模型 `(args,ctx,stdin)→{out,err,code,cd,clear}`，`run` 按 `\|` 切段串流、每段抽重定向)。coreutils：pwd ls(-l 显权限/属主) cd cat(无参透传 stdin) echo mkdir touch rm(软删) mv cp chmod/chown/stat open(启动 App/开文件,子进程挂终端名下) apps ps(PID/PPID) pstree jobs kill(-9/-STOP/-CONT/-TERM) theme env export unset which source clear help。路径走 `vfs.resolvePath/pathOf`、进程/启动走 `sys.*`。文本处理 grep(-inr)/find(-name glob/-type)/wc/head/tail/sort/uniq/cut（读 stdin 或文件、配合管道）。环境/配置 env/export/unset/which/source(`.`别名,带递归守卫)+`$PATH`；`ensureEtcProfile()` 出厂自带 `/etc/profile`，终端 onMount 自动 source 它(改它=持久化 export/启动命令)。下一步 G3.2 见 [[DEVPLAN-LINUX]] |
...
- 🚧 **Phase G 对标 Linux 的系统化补强**（当前自动推进阶段 · 详见 [[DEVPLAN-LINUX]]）：在浏览器沙箱内补齐"像真系统"的能力。**自治开发心跳（定时任务）**按 `DEVPLAN-LINUX.md` 逐项推进：每触发一次 = builder 子 Agent 实现 + supervisor 子 Agent 跑 check/build+审查 + 通过则提交推送+勾选，直到 backlog 清空自删。
  ...
  - 🚧 **G5.1 ✅ 进程模型加深**：Process 加 ppid（终端 open 的子进程挂在终端名下）+ 信号 `kill -9/-STOP/-CONT`（→close/挂起/恢复）+ `ps`/`pstree`/`jobs` + 任务管理器进程树（PPID 列+缩进）。实测 pstree `terminal└─calculator` 树、kill 信号均通。
  - 🚧 **G5.2–G7 待办**：可配置 init + 服务依赖 · 网络工具/man/远程 App 仓库 · 后端账号+同步鉴权。
