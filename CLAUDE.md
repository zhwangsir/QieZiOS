...
| `shell/DesktopPet.svelte` / `system/pet.svelte.ts` | **Live2D 桌面浮层桌宠**：浮在窗口之上(z-9500,Dock/顶栏之下)、可拖、可关；复用 `createPet`。`pet`(persisted: enabled/x/y)。开关：桌面右键「显示/隐藏桌宠」或「伙伴」App 的📌钉到桌面。实测挂载+Pixi 接管 canvas 无报错(视觉真浏览器看) |
...
- 🚧 **Phase D 从 Demo 到平台**（作者反馈"现在像 PPT、自由度低"，点了三件底层大事）。**已拍板：先做 AI、AI 先浏览器直连**。
  ...
  - 🚧 **D3 Live2D 伙伴（基础扎实后解禁，对应方向③）**：✅ 渲染管线(`lib/live2d.ts`：Cubism Core + PixiJS6 + pixi-live2d-display@0.4 懒加载) + ✅ 伙伴窗口 App + ✅ **桌面浮层桌宠**(`DesktopPet.svelte`：可拖/可关,右键或📌钉到桌面)。**待办**：接 D1 的 AI 成「脸」(点它对话/AI 回复时有反应/口型表情)。（曾因「底层太薄」暂停转 Phase E,见记忆 [[foundation-over-features]];基础扎实后续做。）
