...
| `system/sys.ts` | **系统调用门面**(syscall 表雏形)：`sys.proc/fs/ui/bus/log` 收拢内核+系统能力。新代码/AI/iframe 桥应经 `sys.*` 走，旧内置 App 仍直连(待迁)。import 它即拉起「总线→日志」桥。dev 挂 `window.__qzSys` |
...
| `apps/SysMonitor.svelte` | **任务管理器**(Phase E,Dock 📊)：进程页(PID/状态/运行时长 + 聚焦/挂起/结束,真控真进程) + 日志页(klog 实时) + **事件页**(eventLog 原始事件流:名+payload) + 概况页(进程/文件/回收站/已装App/日志/localStorage 计数)。⚠️ 不 import registry(会成环)→ 图标用 appMeta+userApps |
...
- 🚧 **Phase E 内核硬化**（作者要"真正的系统"该有的底层，**当前重心**）。五块按依赖：①syscall 边界 ②IPC/事件总线 ③真进程模型 ④服务层 ⑤可观测性。**作者选了从⑤可观测性切入**。
  ...
  - **待办**：①把内置 App 逐步迁到 `sys.*`(目前旧 App 仍直连内部) ②用户 App 跨沙箱 IPC(`qz.emit`/`qz.on`) ③真进程模型深化(后台服务进程/退出码) ④服务层(通知中心/剪贴板等常驻服务)。
