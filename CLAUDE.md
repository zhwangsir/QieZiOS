...
| `shell/TopBar.svelte` | 顶栏任务栏：**🍆 按钮(开 Launchpad)** + 每窗一个切换 chip（活动高亮/最小化淡显）+ **系统托盘**（明暗切换 🌙/☀️ + 通知中心铃铛 🔔 未读角标 + 历史下拉面板,点外部关闭）+ 实时时钟 |
...
| `shell/Desktop.svelte` | 壁纸 + 景深 + 顶栏 + 桌面图标 + 窗口层 + Dock + 右键菜单 + 全局快捷键(Esc 关窗/Ctrl·Cmd+M 最小化/Alt+\` 轮换/Ctrl·Cmd+K Spotlight/**Ctrl+Alt+方向键平铺活动窗**:←左半→右半↑最大化↓还原)；给 App 传 `data` 启动参数 |
