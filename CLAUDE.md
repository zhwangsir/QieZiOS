...
| `system/sys.ts` | **系统调用门面**(syscall 表雏形)：`sys.proc/fs/ui/bus/log` + `sys.notify(title,opts)`(→emit notify) 收拢内核+系统能力。新代码/AI/iframe 桥应经 `sys.*` 走，旧内置 App 仍直连(待迁)。import 它即拉起「总线→日志」桥。dev 挂 `window.__qzSys` |
| `system/notifications.svelte.ts` / `system/services.ts` | 通知队列(`notifications` + `pushNote`/`dismissNote`,setTimeout 自动消失) / 系统自带服务注册处(**通知中心 `notifyd`**：订阅总线 `notify`+`app.denied` → 弹 toast，演示服务消费事件驱动真实 UI) |
...
- 🚧 **Phase E 内核硬化**（作者要"真正的系统"该有的底层，**当前重心**）。五块按依赖：①syscall 边界 ②IPC/事件总线 ③真进程模型 ④服务层 ⑤可观测性。**作者选了从⑤可观测性切入**。
  ...
  - **待办**：①把内置 App 逐步迁到 `sys.*`(目前旧 App 仍直连内部) ②更多服务(剪贴板/定时器) + 服务退出码/重启 ③IPC 可选加 `ipc` 能力门(目前 IPC 不需声明,仅 `app:` 命名空间不可触系统)。
