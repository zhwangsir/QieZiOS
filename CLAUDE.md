...
- **状态**：Phase A 已提交（`3aba79d`，含 src 全部重写）、Phase B-1 已提交（`0506c58`，会话还原 + 任务栏）；**边缘吸附 snap 已完成并验证（随本次一并提交）**。均**未 push**（作者未授权推送；推送前先问）。
...
src 已全部重写。能力：开/拖/缩放/关窗口、最小化（Dock 或顶栏找回）、最大化/还原（双击标题栏也行）、多窗叠放、点击置顶；**边缘吸附**（拖到左/右/上边缘 → 半屏/最大化，带预览框）；**顶栏任务栏**（每窗一个切换 chip + 实时时钟）；**设置 App 实时改 主色/明暗/圆角/模糊/面板透明度/壁纸**；**整套设置 + 打开的窗口布局都持久化，刷新后原样还原（会话还原）**。换肤只改 CSS 变量 → 0 组件重渲染。
...
| `shell/Window.svelte` | 窗口 chrome：拖/缩放(rAF 批处理)/最大化(CSS 铺满)/最小化/边缘吸附；transform 定位 + `contain` |
...
- 🚧 **Phase B 丝滑外壳**（进行中）：✅ 顶栏任务栏 + ✅ 窗口会话还原 + ✅ 边缘吸附(snap)；**待办**：View Transitions 开关窗动画、键盘焦点/Esc、性能 pass。
