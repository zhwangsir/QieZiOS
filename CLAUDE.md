...
- 🚧 **完善与查漏阶段**（作者 `/loop` 自驱：完善功能 + 查漏 + 继续 · 详见 [[DEVPLAN-POLISH]]，第 2 轮见 [[DEVPLAN-POLISH-2]]）：两路子 Agent 审计出的正确性 bug(P0) + 功能/体验缺口(P1) + 一致性打磨(P2) backlog，每轮挑价值最高项推进。**第 1 轮（DEVPLAN-POLISH）全 ✅；第 2 轮（DEVPLAN-POLISH-2）进行中**：重新审计扩大后代码库得 D1-D5+A3 正确性 + E1-E8 功能缺口。🎉 **第 2 轮全部完成**：正确性 D1-D5 + A3 全清、功能 E1-E8 + E8b（Markdown 预览/图片缩放/计算器键盘/Files 排序视图/终端配色/音效/Dock 自动隐藏/桌面便签/Exposé 任务视图）全做完。性能阶段 P1/P2/P4/P6/P7 完成（仅剩 P5 高风险/P3 远期）。**第 3 轮（[[DEVPLAN-POLISH-3]]）进行中**：重审得 F1-F5 正确性 + G1-G7 功能；**F1（P0 云同步拉取陈旧覆盖，freezePersistence）+ F2（P1 上传同 tick 漏写，flushPersisted await tick）已修**。
