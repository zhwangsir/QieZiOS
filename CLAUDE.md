...
| `apps/Files.svelte` / `apps/TextEdit.svelte` | 文件管理器（面包屑/网格/CRUD/右键）/ 记事本（绑定文件 content 自动存）。**记事本内嵌 AI**：润色/总结/续写/译英 → 流式进预览面板,可替换/追加/丢弃(不毁原文) |
...
- 🚧 **Phase D 从 Demo 到平台**（作者反馈"现在像 PPT、自由度低"，点了三件底层大事）。**已拍板：先做 AI、AI 先浏览器直连**。
  - 🚧 **D1 AI 底层驱动**：✅ 统一 AI 客户端(`system/ai.ts`) **双协议**——anthropic(SDK 直连) + openai 兼容(纯 fetch+SSE,支持 minimax 等推理模型) + ✅ 工具调用驱动系统(`aiTools.ts`) + ✅ 助手 App 流式聊天(含思考过程折叠)。✅ **AI 织进各 App**：记事本(润色/总结/续写/译英)、设置(自然语言 AI 配色)。✅ 浏览器 CORS 用 Vite 同源代理 `/aiproxy` 解决(D0「AI 密钥代理」雏形)。**待办**：接 Spotlight(问 AI)、文件语义搜索、生产环境的真后端代理。
