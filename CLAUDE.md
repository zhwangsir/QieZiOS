...
| `system/sys.ts` | **系统调用门面**(syscall 表)：`sys.proc/fs/ui/clipboard/bus/log` + `sys.notify`。**`aiTools.executeTool` 已迁来走它**(AI/沙箱驱动全经此)。新内置 App 也开始用(Files 复制名称)，旧的逐步迁。import 它即拉起「总线→日志」桥。dev 挂 `window.__qzSys` |
| `system/notifications.svelte.ts` / `system/services.ts` | 通知队列(`notifications` + `pushNote`/`dismissNote`,setTimeout 自动消失) / 系统自带服务注册处(**通知中心 `notifyd`**：订阅 `notify`+`app.denied` → toast；**剪贴板 `clipd`**：订阅 `clip.copy` → 维护历史) |
...
- 🚧 **Phase D 从 Demo 到平台**（作者反馈"现在像 PPT、自由度低"，点了三件底层大事）。**已拍板：先做 AI、AI 先浏览器直连**。
  ...
  - ✅ **D0 地基**：✅ **VFS 二进制**(`blobStore.ts` IndexedDB + `createBinaryFile`/`readBlob`) ✅ **生产 Node 后端**(`server/index.mjs` 零依赖：托管 dist 静态 + 反代 `/aiproxy`→网关,补上 Vite 代理只在 dev 生效的缺口;SSE 透传;可 `AI_KEY` 服务端注入 key。`npm run build && npm run serve`)。**待办**：跨设备同步。
...
- 🚧 **Phase E 内核硬化**（作者要"真正的系统"该有的底层，**当前重心**）。五块按依赖：①syscall 边界 ②IPC/事件总线 ③真进程模型 ④服务层 ⑤可观测性。**作者选了从⑤可观测性切入**。
  ...
  - **待办**：①剩余内置 App 迁 `sys.*`(Files 已起头) ②定时器/提醒服务 + 服务退出码/重启 ③IPC 可选加 `ipc` 能力门。**Phase E 基础已扎实**。
