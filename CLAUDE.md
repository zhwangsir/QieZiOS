...
- **状态**：已提交到 `main`：Phase A `3aba79d`、Phase B（任务栏/会话还原 `0506c58`、边缘吸附 `c41de81`、动画 `93f33c7`、键盘 `16a98e9`）；**Phase C 第一砖 VFS + 文件管理器 + 记事本 已完成并验证（随本次一并提交）**。均**未 push**（作者未授权推送；推送前先问）。
...
src 已全部重写。能力：开/拖/缩放/关窗口、最小化（Dock 或顶栏找回）、最大化/还原（双击标题栏也行）、多窗叠放、点击置顶；**边缘吸附**（拖到左/右/上边缘 → 半屏/最大化，带预览框）；**开关窗 + 最小化动画**（缩放淡入淡出）；**键盘快捷键**（Esc 关窗 / Ctrl·Cmd+M 最小化 / Alt+\` 轮换窗口）+ **活动窗焦点高亮**（主色边框）；**顶栏任务栏**（每窗一个切换 chip + 实时时钟）；**设置 App 实时改 主色/明暗/圆角/模糊/面板透明度/壁纸**；**虚拟文件系统 + 文件管理器 + 记事本**（新建/重命名/删除/进文件夹/双击文本文件编辑，内容自动存）；**整套设置 + 窗口布局 + 文件都持久化，刷新后原样还原**。换肤只改 CSS 变量 → 0 组件重渲染。
...
| `kernel/vfs.svelte.ts` | 虚拟文件系统：inode 风格扁平节点表（parentId 串树）+ `persisted` 持久化 + CRUD API（children/createDir/createFile/rename/remove/pathSegments…） |
...
| `shell/Desktop.svelte` | 壁纸 + 顶栏 + 窗口层(下移避顶栏、`isolate` 隔离层叠、画吸附预览) + Dock + 全局快捷键(`svelte:window`)；给 App 传 `data` 启动参数 |
...
| `apps/Files.svelte` / `apps/TextEdit.svelte` | 文件管理器（面包屑/网格/CRUD）/ 记事本（绑定文件 content 自动存，由 Files 用 `data` 启动） |
...
- 🚧 **Phase C App 平台 + VFS**（进行中）：✅ 虚拟文件系统(localStorage，可换 IndexedDB→OPFS/SQLite-WASM) + ✅ 文件管理器 + ✅ 记事本 + ✅ App 启动参数(`data`)；**待办**：契约式 App SDK(能力声明) + iframe 沙箱(把胡桃博客嵌成 App) + 拖拽移动文件/回收站。
...
## 九、待决策
1. 是否 `git push -u origin main`（push 未授权，先问；已攒 6+ 个 commit）。
2. Phase C 下一项先做哪个：iframe 沙箱(嵌胡桃博客) / 契约式 App SDK / 文件拖拽移动 + 回收站。
