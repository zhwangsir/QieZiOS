...
| `apps/Calculator.svelte` / `apps/Clock.svelte` | 计算器（四则+链式+除零保护 + **键盘输入**(根 div tabindex+onkeydown,数字/运算符/Enter/Backspace/c·Delete,Esc 不拦仍关窗;按钮点击后 focus 收回根) + **计算历史面板**(🕘 切换,最新在前,点 recall 填结果,封顶 30)）/ 时钟（G1 分段切换：**时钟**[SVG 指针]/**秒表**[performance.now+圈速]/**计时器**[倒计时到点 sys.notify+playSound]/**世界时钟**[Intl.DateTimeFormat 7 时区]；50ms tick 仅运行时跑、windowVisible 暂停） |
