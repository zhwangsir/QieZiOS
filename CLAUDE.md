...
| `system/viewport.svelte.ts` | 响应式视口：`matchMedia('(max-width:640px)')` → `viewport.isMobile`（窄屏进移动模式）。Window 铺满窗口、Dock 横滚 |
...
- 🚧 **Phase D 从 Demo 到平台**（作者反馈"现在像 PPT、自由度低"，点了三件底层大事）。**已拍板：先做 AI、AI 先浏览器直连**。
  ...
  - ⏸️ **D3 Live2D 伙伴（已暂停）**：✅ 渲染管线跑通(`Companion.svelte` + `lib/live2d.ts`：Cubism Core + PixiJS6 + pixi-live2d-display@0.4 懒加载单独 chunk；默认 Haru/Cubism4 模型,可换 URL)。⚠️**作者 2026-06-25 反馈「底层太薄弱，Live2D 和其他效果该缓缓」→ D3 及视觉效果暂停，转去夯实内核**（见下 Phase E，及记忆 [[foundation-over-features]]）。
