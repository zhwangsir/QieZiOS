...
- 🚧 **性能 / 存储硬化阶段**（作者拍板「进入下一阶段」选定 · 详见 [[DEVPLAN-PERF]]）：对齐「性能最强 + 最丝滑」+ 解 localStorage 配额迫近天花板的结构性风险。
  ...
  - **待办**：P5 vfs.children() 信号热点（高 blast radius、需专注）/ P2 chat 迁 IDB / P3 OPFS+SQLite-WASM（远期）/ P6 不可见窗 content-visibility。
