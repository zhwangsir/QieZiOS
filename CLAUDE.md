...
| `kernel/services.svelte.ts` | **服务层**：无窗口的常驻后台进程（守护进程）。`registerService`/`startServices`(开机启)/`stopService` + `services` 运行表。「进程≠窗口」：窗口进程在 processes，后台服务在这。服务 `start()` 可返回 stop 清理函数 |
...
| `apps/SysMonitor.svelte` | **任务管理器**(Phase E,Dock 📊)：进程页(PID/状态/运行时长 + 聚焦/挂起/结束,真控真进程) + 日志页(klog 实时) + **事件页**(eventLog 原始事件流:名+payload) + 概况页(进程/服务/文件/回收站/已装App/日志/localStorage 计数)。进程页含**后台服务**小节(无窗口进程,可停)。⚠️ 不 import registry(会成环)→ 图标用 appMeta+userApps |
...
- 🚧 **Phase E 内核硬化**（作者要"真正的系统"该有的底层，**当前重心**）。五块按依赖：①syscall 边界 ②IPC/事件总线 ③真进程模型 ④服务层 ⑤可观测性。**作者选了从⑤可观测性切入**。
  ...
  - **待办**：①剩余内置 App 迁 `sys.*`(Files 已起头) ②服务退出码/重启/健康检查 ③IPC 可选加 `ipc` 能力门。**Phase E 基础已扎实**。
