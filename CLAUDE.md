...
| `lib/shell.ts` | **Shell 引擎**(对标 Linux · 终端用)：分词(尊重引号) + `$VAR` 替换 + 重定向 `>`/`>>` + 命令表 `run(line,ctx)→{out,err,code,cd,clear}`。coreutils：pwd ls(-l) cd cat echo mkdir touch rm(软删) mv cp open(启动 App/开文件) apps ps kill theme env export clear help。路径走 `vfs.resolvePath/pathOf`、进程/启动走 `sys.*`。管道/grep 等见 [[DEVPLAN-LINUX]] Phase G2 |
