...
- 🚧 **完善与查漏阶段**（作者 `/loop` 自驱：完善功能 + 查漏 + 继续 · 详见 [[DEVPLAN-POLISH]]，第 2 轮见 [[DEVPLAN-POLISH-2]]）：两路子 Agent 审计出的正确性 bug(P0) + 功能/体验缺口(P1) + 一致性打磨(P2) backlog，每轮挑价值最高项推进。**第 1 轮（DEVPLAN-POLISH）全 ✅；第 2 轮（DEVPLAN-POLISH-2）进行中**：重新审计扩大后代码库得 D1-D5+A3 正确性 + E1-E8 功能缺口。🎉 **第 2 轮全部完成**：正确性 D1-D5 + A3 全清、功能 E1-E8 + E8b（Markdown 预览/图片缩放/计算器键盘/Files 排序视图/终端配色/音效/Dock 自动隐藏/桌面便签/Exposé 任务视图）全做完。性能阶段 P1/P2/P4/P6/P7 完成（仅剩 P5 高风险/P3 远期）。**第 3 轮（[[DEVPLAN-POLISH-3]]）进行中**：重审得 F1-F5 正确性 + G1-G7 功能；**正确性 F1-F5 全清**——F1（P0 云同步拉取陈旧覆盖，freezePersistence）+ F2（P1 上传同 tick 漏写，flushPersisted await tick）+ **F4（P2 计算器/图片查看器 onKey 放行修饰键组合，不再吞 Cmd/Ctrl+C·Cmd+0/-）+ F5（P2 resetDock 复位 autohide）已修**；功能已完成 G1（时钟秒表/计时器/世界时钟）+ G2（Files 详情/信息面板：右侧 aside，路径/精确字节/时间戳/类型 mime/属主权限/图片缩略图）+ G3（计算器科学模式：`lib/calc.ts` 安全递归下降求值非 eval + 标准/科学切换 + 函数/常量/阶乘/括号/内存键）+ G4（媒体音视频查看器：MediaViewer `<audio>`/`<video>` + blob objectURL，Files/桌面/shell open/Spotlight 四处按 mime 统一分流）+ G5（界面字体族）+ G7（记事本导出/另存下载：⬇导出原始文本 + md 导出 HTML 文档复用 renderMarkdown，打通 VFS→本机出口），仅剩 G6（截图工具）。
