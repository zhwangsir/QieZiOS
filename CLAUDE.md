...
| 文件 | 作用 |
|---|---|
| `kernel/processes.svelte.ts` | 内核进程表（`persisted` → 会话还原）+ `launch/close/focus/minimize/restore/toggleMaximize/setBounds/activeId/cycleWindows`；进程含 **`pid`**(每次开机重排)+**`startedAt`**+`data` 启动参数；生命周期事件写 `logSys` |
| `kernel/log.svelte.ts` | **内核日志**(可观测性地基)：`klog`(响应式、不持久化=dmesg 语义、封顶 500 条) + `logSys(source,msg,level)`。进程/VFS/AppSDK/开机 init 在真实事件点埋点写入 |
...
| `apps/SysMonitor.svelte` | **任务管理器**(Phase E,Dock 📊)：进程页(PID/状态/运行时长 + 聚焦/挂起/结束,真控真进程) + 日志页(klog 实时,自动滚) + 概况页(进程/文件/回收站/已装App/日志/localStorage 计数)。⚠️ 不 import registry(会成环)→ 图标用 appMeta+userApps |
...
- 🚧 **Phase E 内核硬化**（作者要"真正的系统"该有的底层，**当前重心**）。五块按依赖：①syscall 边界 ②IPC/事件总线 ③真进程模型 ④服务层 ⑤可观测性。**作者选了从⑤可观测性切入**。
  - ✅ **可观测性**：`kernel/log.svelte.ts`(dmesg 式系统日志,真实事件埋点) + 进程模型补 `pid`/`startedAt` + **任务管理器 App**(`SysMonitor.svelte`：进程/日志/概况三页,真控真进程)。实测：开机 init 序列入日志、建文件/启动/结束都实时记录、任务管理器结束进程真生效。
  - **待办**：①统一 syscall 边界(内置 App 不再直接 import 内部,能力对所有 App 强制) ②IPC/事件总线(pub/sub + App 间消息) ③真进程模型深化(后台服务进程/挂起/退出码) ④服务层(通知中心/剪贴板等常驻服务)。
