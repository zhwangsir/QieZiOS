...
| `system/dnd.svelte.ts` | **勿扰 / 专注模式**(R5-F3,持久 `qz.dnd`)：`dnd.enabled` + `toggleDnd`。开启后 `pushNote` 跳过 live toast（仍入 noteHistory 不丢）、`playSound` 静音全部系统音；顶栏 🤫 切换。不入 SETTINGS_KEYS、随 qz.* 同步 |
...
| `system/theme.svelte.ts` | 把设置算成 CSS token，`applyTokens()` 写进 `:root`。**G5 字体维度**：`settings.fontFamily` + `FONT_FAMILIES`(系统/无衬线/衬线/等宽/圆体,栈锚定通用族)/`fontStack()` → `--qz-font` token → `app.css` body `font-family:var(--qz-font)`；换字体 0 组件重渲染；终端等 `font-mono` 显式字体不受影响。**R4-F3 主色渗透**：`tintMix(base,accent,t)` 用 `color-mix` 把主色按 `settings.accentTint`(0~0.15) 拌进 `--color-qz-surface`/`--color-qz-elevated`（t=0 返回原色零回归；qz-glass 再嵌套一层 color-mix 正常解析）。**R5-F1 自动明暗**：`resolvedMode()` 把 `settings.mode`(dark/light/**auto**/**schedule**) 解析成明/暗——auto 跟随 `viewport.systemDark`、schedule 按 lightStart/darkStart 时段(模块级 `$effect.root` 每分钟 tick、仅 schedule 武装)；`activeTokens`/App.colorScheme 都用它 |
| `system/settings.svelte.ts` | 用户设置（持久化）+ 主色预设 |
| `system/recents.svelte.ts` | **最近使用**(R4-F2,持久 `qz.recents`)：`recents={files,apps}` 环形封顶 20 + `pushFront` 置顶去重 + pushRecentFile/pushRecentApp/forgetRecent。由 `sys.openApp` chokepoint 推进（文件查看器+string data→最近文件；非 hidden App→最近 App）。Spotlight 空查询置顶最近文件 + 按最近度浮排 App |
| `system/appPrefs.svelte.ts` | **每 App 偏好**(R4-F5,持久 `qz.appprefs`)：按 appId 存默认窗口尺寸 `{w,h}`。`setAppSize`/`getAppPref`/`clearAppPref`；`sys.openApp` 启动尺寸回退 `appPrefs → appMeta`；窗口标题栏右键「保存当前大小为默认」/「重置默认大小」。随 qz.* 同步、不入 SETTINGS_KEYS |
...
| `shell/Window.svelte` | 窗口 chrome：拖/缩放(rAF 批处理；**R4-F1 全向缩放**：四边+四角 8 手柄 `RESIZE_HANDLES`，`startResize(e,dir)` 方向化，左/上边固定对边调 x/y、min 280×180 夹时对边不漂)/最大化(CSS 铺满)/最小化/边缘吸附(左/右半屏 + 上=最大化 + **四角=四分之一屏** tl/tr/bl/br)/开关动画/活动窗焦点边框；transform 定位 + `contain`。**拖拽中被卸载**(AI/快捷键关窗)→ `onDestroy` 兜底 cancelAnimationFrame + 清残留 snap 预览(仅本窗在拖时)。**移动模式**(`viewport.isMobile`)：窗口铺满 + 禁拖拽缩放，靠顶栏/Dock 切换。**经 `provideWindow`(`lib/winctx.ts`)把最小化状态传给窗口内 App** → 时钟/任务管理器/提醒最小化时暂停每秒定时器(`windowVisible()` 守卫 effect)，还原自动恢复 |
| `system/viewport.svelte.ts` | 响应式视口：`matchMedia` → `viewport.isMobile`（窄屏移动模式）+ `viewport.reducedMotion`（系统「减少动态」偏好）+ `viewport.systemDark`（`prefers-color-scheme: dark`，R5-F1 mode='auto' 跟随它）。isMobile→Window 铺满/Dock 横滚；reducedMotion→app.css 关过渡 + pop/fly 置 0 |
| `lib/motion.ts` | 窗口开/关的 Svelte 自定义过渡 `pop`（只动 opacity + 独立 scale 属性，不碰 transform） |
| `shell/WindowControls.svelte` | 红绿灯：关 / 最小化 / 最大化。**R4-F8 贴靠布局浮层**：悬停最大化键(`group-hover/max`)出 12 区选择器(半 ◧◨⬒⬓ / 四分 ◰◳◱◲ / 三分 / ⛶ 最大化)，点 zone→`onTile(zone)`→ Window `tileTo` 算窗口层区域 setBounds |
| `shell/TopBar.svelte` | 顶栏任务栏：**🍆 按钮(开 Launchpad)** + 每窗一个切换 chip（活动高亮/最小化淡显 + **右键菜单**:最小化·还原/最大化·还原大小/关闭,与 Dock/Window 一致）+ **系统托盘**（明暗切换 🌙/☀️ + **快捷设置 ⚙️**(R5-F2 QuickSettings) + 通知中心铃铛 🔔 未读角标 + 历史下拉面板,点外部关闭）+ 实时时钟 |
| `shell/QuickSettings.svelte` | **顶栏快捷设置面板**(R5-F2)：⚙️ 下拉，自管 open + svelte:window 点外部关。收纳 外观(明/暗/自动/定时)、勿扰(dnd)、声音开关+音量(soundPrefs)、主色 swatch(accentPresets)、下一张壁纸(循环 wallpaperId+清 customWallpaper)。多为已有状态接线，少开 Settings App |
...
| `shell/Widgets.svelte` / `shell/widgetState.svelte.ts` | **桌面活动小组件**(R4-F6)：贴桌面的小卡片，持久 `qz.widgets`(Widget{id,kind,x,y})，kind=时钟(复用 Clock SVG 表盘)/日历(月历今日高亮)/系统状态(进程数)。单个 1s `now` tick 经 `$effect` 仅 `list.length>0` 时武装(增删自动重武装、移动不重武装、cleanup 清理无泄漏)；拖动镜像 StickyNotes 夹视口；手柄 ⟳ 切类型 + ✕ 移除。桌面右键「新建小组件」。`addWidget/removeWidget/cycleWidgetKind`。⚠️ 文件名 widgetState 避免与组件 Widgets.svelte 在 Windows 撞名 |
...
| `shell/Expose.svelte` / `shell/exposeState.svelte.ts` | **任务视图 Exposé**(E7,F3 唤起/桌面右键/Shortcuts)：全屏 backdrop-blur 浮层把所有窗口(含最小化)做成卡片网格(图标+标题+最小化 badge)，点卡 `restore(id)` 还原+聚焦+关，点遮罩/Esc/F3 关。非破坏性(不重排真窗口，与 B13 tileGrid 互补)；z-[10000] 盖窗口/Dock 低于通知。仿 launchpadState 模式 |
...
| `shell/Spotlight.svelte` / `shell/spotlightState.svelte.ts` | 命令面板（Ctrl/Cmd+K）：搜内置 App + **已装用户 App** + VFS 文件（**名字+正文，正文命中显片段**）+ **动作命令**(切换明暗/清空回收站/打开终端·设置/显示桌面/关闭所有窗口) + **「问 AI」入口**(选中→开助手并以 `data.ask` 自动提问) + **最近文件/最近 App**(R4-F2：空查询置顶 recents) + **内联计算器**(R4-F4：query 像算式→置顶结果、Enter 复制，复用 `lib/calc.ts`)，方向键/Enter 启动 |
...
| `apps/Welcome.svelte` / `apps/Settings.svelte` | 欢迎 App（R4-F9 快速导览：4 张卡片点开 文件/终端/设置/应用商店 + Spotlight 提示 + 窗口手感提示，替代原点击计数 demo）/ 设置 App（自定义闭环）。设置含 **AI 全配置**(provider 切换 + 一键预设 + key/地址/模型/人设) + **AI 配色**(一句话换肤,复用 set_theme 工具) + 主色/明暗/圆角/模糊/透明度/**主色渗透**(R4-F3)/界面缩放/字体/壁纸/自定义 CSS/命名主题预设 |
| `apps/Files.svelte` / `apps/TextEdit.svelte` | 文件管理器（面包屑/网格/CRUD/右键 + **即时名字过滤** + **AI 语义搜索** + **⬆上传二进制**(createBinaryFile→IndexedDB) + 双击图片开图片查看器 + **复制/剪切/粘贴**(右键 + 工具栏粘贴 + Ctrl+C/X/V,copy 走 copyNode 递归深拷贝、cut 走 move) + **多选**(点击/Ctrl/Shift,选中态 ring,批量 删除/复制/剪切,Delete 键,操作条 + **R4-F7 框选 marquee**:空白拖框 AABB 命中,与 item 拖拽/click 共存、suppressClear 防框选后清空) + **排序/视图**(E4：排序 名称/类型/大小/修改时间 + 升降序，网格/列表视图切换；list 显大小·mtime·属主权限列，窄窗渐次藏列；文件夹恒在前；同一 item 包裹单一交互来源，重命名输入用 snippet 复用) + **详情/信息面板**(G2：工具栏 ℹ 切换 / 右键「显示简介」开右侧可折叠 `<aside>`；选中单个项显 类型(mime/扩展)/完整路径 pathOf/大小(文件精确字节·文件夹 N 项)/创建·修改全时间戳/属主/权限 rwx+八进制/你的权限，图片 `$effect` readBlob→objectURL 出缩略图带 revoke 防泄漏；内容区+面板 `flex` 并排 `min-w-0` 防溢出)）/ 记事本（绑定文件 content 自动存 + **Ctrl+F 查找/替换**：计数/上下导航环绕/区分大小写/替换·全部，gated on writable + 底部行·字符状态栏 + **Markdown 预览**：`.md`/`.markdown` 文件显「编辑/预览」切换,预览态 `{@html renderMarkdown}`(复用 AI 回复同款安全渲染器),Ctrl+F 自动切回编辑 + **导出/另存下载**（G7：常驻顶部工具条「⬇ 导出」原始文本[.md→text/markdown 其余→text/plain]、md 额外「HTML」导出[renderMarkdown 渲染包进完整 `<!doctype html>` 文档+内联 style 映射其 Tailwind 类→独立打开排版正常,title 经 esc 转义]；`downloadBlob` 标准 `<a download>`+1000ms revoke；不门控 writable））。**记事本内嵌 AI**：润色/总结/续写/译英 → 流式进预览面板,可替换/追加/丢弃(不毁原文) |
...
| `apps/MediaViewer.svelte` | 媒体查看器(隐藏 app `mediaviewer`,G4)：音/视频播放。`readBlob`→`URL.createObjectURL`→ 视频 `<video controls autoplay>` / 音频 `<audio controls autoplay>`+🎵 占位，loading/ok/error 三态 + 信息条(图标/名/大小)，`$effect` cleanup `active` 守卫+revoke 防泄漏(克隆 ImageViewer 范式)。vfs `isAudio`/`isVideo`/`isMedia` 判型(mime 或扩展名)；Files 双击 / DesktopIcons / shell `open` / Spotlight 四处统一按类型分流(图片→imageviewer / 音视频→mediaviewer / 其余→textedit)；Files 上传 accept 含 audio·video |
| `apps/Screenshot.svelte` | 截图工具(可见 app `screenshot` 📸,G6)：`getDisplayMedia({video})` 抓一帧→`<video>`(loadedmetadata 拿尺寸,不用 rAF 避后台冻结,1s 兜底)→drawImage 进 canvas→停流→`toBlob('image/png')`→预览 `<img>`+尺寸；「存到文件」`createBinaryFile('root',…)` 存 PNG 进 VFS、「下载」`<a download>`+1s revoke。idle/capturing/preview/error 四态 + supported 守卫 + 时间戳名 + 取消选择器静默回 idle + `$effect` cleanup revoke 预览 URL 且停活动流(卸载不漏)。⏳ 真实屏幕选择器授权待真机(无头 stub captureStream 验证全管线) |
...
| `apps/Calculator.svelte` / `lib/calc.ts` / `apps/Clock.svelte` | 计算器（**标准/科学 双模式**(G3)：标准=四则+链式+除零保护(原状态机)、科学=表达式录入经 `lib/calc.ts` 安全递归下降求值(**非 eval**：+−×÷·^右结合·一元·括号·阶乘!·函数 sin/cos/tan/ln/log/√/exp/abs·常量 π/e·内存 MC/MR/M+/M-；优先级一元在幂之上→-2^2=-4；n>170 阶乘→Infinity 防 DoS；显示符号归一化) + **键盘输入**(根 div tabindex+onkeydown 按模式分发,Esc 不拦仍关窗,修饰键组合放行;按钮点击后 focus 收回根) + **计算历史面板**(🕘 切换,最新在前,点 recall 按模式填回,封顶 30)。⚠️ 续算正则须含 U+2212「−」否则减号丢结果）/ 时钟（G1 分段切换：**时钟**[SVG 指针]/**秒表**[performance.now+圈速]/**计时器**[倒计时到点 sys.notify+playSound]/**世界时钟**[Intl.DateTimeFormat 7 时区]；50ms tick 仅运行时跑、windowVisible 暂停） |
...
### UI/UX 改造进度（2026-07-18）
- 🎉 **UI/UX 改造 M1–M5 全部完成**：M1 视觉地基（Lucide 图标三层体系 / 玻璃 saturate+高光+可选折射 / Inter 字体）→ M2 动效（弹簧化 / Genie 神灯最小化 / Dock 高斯波形）→ M3 仪式感（boot/login/lock 会话状态机 + 顶栏系统/App 菜单 + 关机流程）→ M4 收尾（QuickSettings 控制中心化 + 全屏三页首秀引导 + 移动端图标长按菜单 + 图标映射去重）+ M5 移动端 iOS 化外壳（状态栏/Home/托盘/Indicator/控制中心/锁屏/灵动岛）。vitest 7 文件 60 例全绿、check 0 错 0 警、build 通过。各里程碑实施记录与待真机验证项汇总见 [[DEVPLAN-UIUX]]。
...
- 🚧 **性能 / 存储硬化阶段**（作者拍板「进入下一阶段」选定 · 详见 [[DEVPLAN-PERF]]）：对齐「性能最强 + 最丝滑」+ 解 localStorage 配额迫近天花板的结构性风险。
  ...
  - ✅ **P6 最小化窗 content-visibility**：最小化窗口保持挂载（opacity:0 平滑动画），其内容区加 `content-visibility:hidden`（proc.minimized 时）→ 浏览器跳过最小化窗内容布局/绘制；flex-1 防盒塌陷、hidden 保滚动状态、与 windowVisible 暂停定时器正交。非最小化字节级零回归。
  - **待办**：P5 vfs.children() 信号热点（高 blast radius、需专注全验）/ P3 OPFS+SQLite-WASM（远期）。
- 🚧 **完善与查漏阶段**（作者 `/loop` 自驱：完善功能 + 查漏 + 继续 · 详见 [[DEVPLAN-POLISH]]，第 2 轮见 [[DEVPLAN-POLISH-2]]）：两路子 Agent 审计出的正确性 bug(P0) + 功能/体验缺口(P1) + 一致性打磨(P2) backlog，每轮挑价值最高项推进。**第 1 轮（DEVPLAN-POLISH）全 ✅；第 2 轮（DEVPLAN-POLISH-2）进行中**：重新审计扩大后代码库得 D1-D5+A3 正确性 + E1-E8 功能缺口。🎉 **第 2 轮全部完成**：正确性 D1-D5 + A3 全清、功能 E1-E8 + E8b（Markdown 预览/图片缩放/计算器键盘/Files 排序视图/终端配色/音效/Dock 自动隐藏/桌面便签/Exposé 任务视图）全做完。性能阶段 P1/P2/P4/P6/P7 完成（仅剩 P5 高风险/P3 远期）。🎉 **第 3 轮（[[DEVPLAN-POLISH-3]]）全部完成**：F1-F5 正确性 + G1-G7 功能全 ✅。正确性 F1（P0 云同步拉取陈旧覆盖，freezePersistence）+ F2（P1 上传同 tick 漏写，flushPersisted await tick）+ F4（计算器/图片查看器 onKey 放行修饰键）+ F5（resetDock 复位 autohide）；功能 G1（时钟秒表/计时器/世界时钟）+ G2（Files 详情/信息面板）+ G3（计算器科学模式：`lib/calc.ts` 安全递归下降求值非 eval）+ G4（媒体音视频查看器 MediaViewer，四处按 mime 统一分流）+ G5（界面字体族）+ G6（截图工具：getDisplayMedia→canvas→PNG 存 VFS+下载，⏳ 真实选择器待真机）+ G7（记事本导出/另存下载）。其中 G3 续算减号、G4 Spotlight 漏分流、G6 流卸载泄漏由 supervisor 子 Agent 抓出并修。**第 4 轮（[[DEVPLAN-POLISH-4]]）进行中**：两路子 Agent 重审得 C1-C4 正确性 + F1-F10 功能；**R4-C1（shell `>` 重定向到二进制文件损坏节点+泄漏 blob）+ R4-F1（窗口全向缩放：四边四角 8 手柄）+ R4-C2（OpenAI 流式 reader 泄漏：try/finally+reader.cancel）+ R4-F2（最近文件/App：`system/recents.svelte.ts` + sys.openApp chokepoint 记录 + Spotlight 空查询置顶最近文件/最近 App 浮前）+ R4-C3（终端历史 ↑/↓ 防御性夹索引，复审为已安全）+ R4-C4（桌宠拖动/缩窗夹回视口防取不回：clampPet 四边夹 + onMount/resize 回收）+ R4-F4（Spotlight 内联计算器：query 像算式时置顶结果、Enter 复制，复用 lib/calc.ts）+ R4-F3（主题 accent tint：`settings.accentTint` 0~0.15 + `theme.tintMix` 用 color-mix 把主色拌进 surface/elevated + Settings「主色渗透」滑块，0=零回归）+ R4-F9（空状态+引导打磨：Welcome 改 4 卡片快速导览[去点击计数 demo]、Files/Trash 空状态加图标+提示+主操作按钮）+ R4-F6（桌面小组件层：`shell/widgetState.svelte.ts`+`Widgets.svelte` 时钟/日历/系统状态 拖拽卡，镜像 StickyNotes 持久化，桌面右键「新建小组件」）+ R4-F5（每 App 默认窗口尺寸：`system/appPrefs.svelte.ts` 按 appId 存 w/h，sys.openApp 回退，窗口标题栏右键「保存当前大小为默认」/「重置默认大小」）+ R4-F7（Files 框选 marquee：内容区空白拖框 AABB 命中 [data-file-id]，与 item HTML5 拖拽/click 共存，suppressClear 防框选后清空）+ R4-F8（窗口贴靠布局弹窗：WindowControls 悬停最大化键出 12 区浮层[半/四分/三分/最大化]，Window.tileTo 算窗口层区域 setBounds）已修**（正确性 C1-C4 全清）。仅剩 F10（神灯动画，真机验）——无头可验的第 4 轮 backlog 已清空。**第 5 轮（[[DEVPLAN-POLISH-5]]）进行中**：两路子 Agent 重审（重点第 4 轮新增）得 C1-C5 正确性 + F1-F8 功能；**R5-C1（桌面小组件取不回：clampAll）+ R5-F1（自动明暗：mode 扩 dark/light/auto/schedule + viewport.systemDark + theme.resolvedMode[auto 跟随系统/schedule 按时段]+ App colorScheme 用 resolvedMode + Settings 四段+定时输入）已修**。**R5-C2-C5 已批量修**（C2 forgetRecent 经 setRecentForgetter 注入接进 vfs.purge / C3 calc 多小数点抛错 / C4 计算器 ±·% 对错误 isFinite 守卫 / C5 窗口 west 缩放 x 加 max(0) 下夹）→ 第 5 轮正确性 C1-C5 全清。**R5-F1（自动明暗）+ R5-F3（勿扰）+ R5-F2（顶栏快捷设置面板 `shell/QuickSettings.svelte`：⚙️ 下拉收纳 外观/勿扰/声音+音量/主色/下一张壁纸；勿扰从独立 🤫 移入此面板）已修**。剩 F4（Files 键盘导航+F2+Ctrl+D）/F5（壁纸轮播）/F6（拖文件进 App）/F7（更多小组件）/F8（窗口置顶）。
