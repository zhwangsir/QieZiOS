...
| `system/theme.svelte.ts` | 把设置算成 CSS token，`applyTokens()` 写进 `:root`。**G5 字体维度**：`settings.fontFamily` + `FONT_FAMILIES`(系统/无衬线/衬线/等宽/圆体,栈锚定通用族)/`fontStack()` → `--qz-font` token → `app.css` body `font-family:var(--qz-font)`；换字体 0 组件重渲染；终端等 `font-mono` 显式字体不受影响 |
