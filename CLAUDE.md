...
- **状态**：已 **push 到 `origin/main`（同步）**。包含 Phase A `3aba79d`、Phase B、Phase C VFS `06847ec`、打磨①②③④、回收站 `8cd017e`、Spotlight、文件拖拽移动、主题编辑器、**Phase D 的 AI 助手 MVP**。**作者已给全部授权 → 可自由 commit + push，不再每次问**。
...
| `system/ai.ts` | AI 引擎：Anthropic 客户端(浏览器直连，baseURL/model/maxTokens/人设可配) + 流式 + 工具调用 agent loop；**SDK 懒加载**(动态 import) + 可中断(AbortSignal) |
| `system/assistantChat.svelte.ts` | 助手对话(`persisted` → 关窗/刷新不丢) |
...
| `system/aiConfig.svelte.ts` | AI 配置（持久化）：API key + 模型（opus-4-8 / haiku-4-5） |
...
| `apps/Assistant.svelte` | AI 助手 App：流式聊天(Markdown 渲染) + 工具 chip + 停止/清空 + 对话持久化 + 无 key 提示 |
...
- 🚧 **Phase D 从 Demo 到平台**（作者反馈"现在像 PPT、自由度低"，点了三件底层大事）。**已拍板：先做 AI、AI 先浏览器直连**。
  - 🚧 **D1 AI 底层驱动**：✅ MVP——统一 AI 客户端(`system/ai.ts`，Claude opus-4-8/haiku-4-5，浏览器直连用户自填 key) + ✅ 工具调用驱动系统(`aiTools.ts`：launch_app/VFS CRUD/set_theme) + ✅ 助手 App 流式聊天。**待办**：把 AI 织进各 App(记事本润色/设置自然语言换肤/文件语义搜索)、接 Spotlight。
  - **D0 地基**：VFS 二进制(localStorage→IndexedDB，给 Live2D 模型/图片/App 资源) + 最小 Node 后端(AI 密钥代理 + 同步)。
  - **D2 App 开发平台**：契约式 App SDK(iframe + postMessage RPC + 能力声明) + 系统内代码编辑器(CodeMirror) → 在系统里写/跑/存 App；胡桃博客以后也走 iframe 嵌。
