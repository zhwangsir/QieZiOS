...
| `system/appSdk.ts` | **App SDK 契约运行时**(D2 地基)：`buildSrcdoc`(用户代码 + 注入访客 SDK `window.qz`) + `handleGuestCall`(宿主端 RPC 闸：校验能力白名单 `DEFAULT_CAPS` → 路由到 executeTool / `complete` AI → postMessage 回灌)。用户 App 跑沙箱 iframe(`sandbox=allow-scripts`,无 same-origin),只能经 postMessage 调系统 |
...
| `shell/Spotlight.svelte` / `shell/spotlightState.svelte.ts` | 命令面板（Ctrl/Cmd+K）：搜 App 与 VFS 文件 + **「问 AI」入口**(选中→开助手并以 `data.ask` 自动提问)，方向键/Enter 启动 |
...
| `apps/Studio.svelte` | **开发者 App**（D2）：左编辑器(textarea,Tab 缩进) + 右沙箱 iframe 预览 + ▶运行/重置/SDK 说明。宿主 `message` 监听只收自己 iframe(`e.source===contentWindow`)→ `handleGuestCall`。⚠️ run 用 `setTimeout` 重建 iframe(rAF 在隐藏标签页会冻结) |
...
- 🚧 **Phase D 从 Demo 到平台**（作者反馈"现在像 PPT、自由度低"，点了三件底层大事）。**已拍板：先做 AI、AI 先浏览器直连**。
  ...
  - 🚧 **D2 App 开发平台**：✅ 契约式 App SDK 跑通(`appSdk.ts`：沙箱 iframe + postMessage RPC + 能力白名单 → 复用 executeTool/AI) + ✅ 开发者 App(`Studio.svelte`：编辑器 + 实时沙箱预览 + 注入 `window.qz`)。**实测**：iframe 里 `qz.launchApp('calculator')` → 经 RPC 真开出窗口。**待办**：编辑器升级 CodeMirror(语法高亮) + 把用户 App 存成命名 App(注册到 Dock/启动器) + 能力声明 UI；胡桃博客以后也走 iframe 嵌。
