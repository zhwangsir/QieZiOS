...
- 🚧 **Phase D 从 Demo 到平台**（作者反馈"现在像 PPT、自由度低"，点了三件底层大事）。**已拍板：先做 AI、AI 先浏览器直连**。
  ...
  - 🚧 **D3 Live2D 伙伴**：✅ 渲染管线跑通(`Companion.svelte` + `lib/live2d.ts`：Cubism Core + PixiJS6 + pixi-live2d-display@0.4 懒加载单独 chunk；默认 Haru/Cubism4 模型,可换 URL)。实测 core/pixi/WebGL/模型加载全过、无报错(视觉待真浏览器确认)。**待办**：绑定 D1 的 AI 成「脸」(说话/口型/表情) + 做成桌面浮层桌宠(现在是窗口) + 自定义模型走二进制 VFS。 |
